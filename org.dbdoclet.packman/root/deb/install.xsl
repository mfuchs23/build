...
if [ "/</xsl:text>
  ...
  dummy=""
    ...
    <xsl:text>if [ -f debian/</xsl:text>
    <xsl:value-of select="$pkgname"/>   
    <xsl:text>.desktop ] ; then&#xa;</xsl:text>
    <xsl:text>  install -d debian/</xsl:text>
    <xsl:value-of select="$pkgname"/>   
    <xsl:text>/usr/share/applications&#xa;</xsl:text>
    <xsl:text>  install -m 0644  debian/</xsl:text>
    <xsl:value-of select="$pkgname"/>   
    <xsl:text>.desktop debian/</xsl:text>
    <xsl:value-of select="$pkgname"/>   
    <xsl:text>/usr/share/applications&#xa;</xsl:text>
    <xsl:text>fi&#xa;</xsl:text>
  </xsl:template>
    ...
    <xsl:for-each select="file">
     <xsl:variable name="recursive">
      ...
      <xsl:variable name="target">
        <xsl:value-of select="substring($buildroot, 1, string-length($buildroot) - 1)"/>
